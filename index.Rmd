...
* **Variable selection**: the goal is to select the ``relevant'' predictors for each dataset. Data were generated as $y = X\beta + \varepsilon$
with $S = \{j: \beta_j \neq 0\}$ and $N = \{j: \beta_j = 0\}$. Your selection $\hat{S}$ will be evaluated by the Matthews correlation coefficient.

   * Data set [VS](docs/data/VS.txt) with $n=40$ and $d= 19$
   * Submission example: [TXT](docs/data/2575_VS.txt) and [PDF](docs/data/2575_VS_SUPPL.pdf).
